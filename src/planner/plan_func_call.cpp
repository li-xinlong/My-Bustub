...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() == 1) {
      ...
      if (func_name == "lower") {
        std::shared_ptr<StringExpression> ptr = std::make_shared<StringExpression>(arg, StringExpressionType::Lower);
        return ptr;
      } else {
        std::shared_ptr<StringExpression> ptr = std::make_shared<StringExpression>(arg, StringExpressionType::Upper);
        return ptr;
      }
    }
  }
