...
class WindowFunctionExecutor : public AbstractExecutor {
 public:
  ...
  // auto MakeAggregateKey(const Tuple *tuple, std::vector<AbstractExpressionRef> &group_bys_) -> AggregateKey {
  //   std::vector<Value> keys;
  //   for (const auto &expr : group_bys_) {
  //     keys.emplace_back(expr->Evaluate(tuple, GetOutputSchema()));
  //   }
  //   return {keys};
  // }

  // /** @return The tuple as an AggregateValue */
  // auto MakeAggregateValue(const Tuple *tuple, AbstractExpressionRef &agg_expr_) -> AggregateValue {
  //   std::vector<Value> vals;
  //   vals.emplace_back(agg_expr_->Evaluate(tuple, GetOutputSchema()));
  //   return {vals};
  // }
  // auto whatType(WindowFunctionType &function_type, AggregationType &aggregationtype, Value &value_begin, bool
  // &is_rank);
  auto Equal(const Tuple &a, const Tuple &b,
  ...
  auto MakeAggregateKey(const Tuple *tuple, std::vector<AbstractExpressionRef> &group_bys_,
                        const Schema &output_schema_) -> AggregateKey;
  auto MakeAggregateValue(const Tuple *tuple, AbstractExpressionRef &agg_expr_,
                          const Schema &output_schema_) -> AggregateValue;
  auto GenerateAggregationType(AggregationType &agg_type, Value &default_value,
 ...
 private:
  ...
  std::unordered_set<uint32_t> column_ids;
};
}  // namespace bustub
